    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('school.players.store') }}">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="date_of_birth" id="date_of_birth" 
                                   ...
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                   required>
                            <p class="mt-1 text-sm text-gray-500">Age category (U13, U15, U17, U19) will be calculated automatically.</p>
                            @error('date_of_birth')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="player_category" id="player_category" 
                                ...
                                @foreach($playerCategories as $category)
                                    <option value="{{ $category }}" {{ old('player_category') == $category ? 'selected' : '' }}>
                                        {{ $category }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="batting_style" id="batting_style" 
                                ...
                                @foreach($battingStyles as $style)
                                    <option value="{{ $style }}" {{ old('batting_style') == $style ? 'selected' : '' }}>
                                        {{ $style }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="bowling_style" id="bowling_style" 
                                ...
                                @foreach($bowlingStyles as $style)
                                    <option value="{{ $style }}" {{ old('bowling_style') == $style ? 'selected' : '' }}>
                                        {{ $style }}
                                    </option>
                                @endforeach
                            </select>
