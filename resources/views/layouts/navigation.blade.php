            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->role === 'ADMIN')
                        ...
                        <x-nav-link :href="route('admin.schools.pending')" :active="request()->routeIs('admin.schools.pending')">
                            {{ __('Pending Schools') }}
                        </x-nav-link>
                    ...
                    @if(Auth::user()->role === 'SCHOOL')
                        ...
                        <x-nav-link :href="route('school.players.index')" :active="request()->routeIs('school.players.*')">
                            {{ __('Players') }}
                        </x-nav-link>
                        <x-nav-link :href="route('school.profile.index')" :active="request()->routeIs('school.profile.*')">
                            {{ __('School Profile') }}
                        </x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->role === 'ADMIN')
                ...
                <x-responsive-nav-link :href="route('admin.schools.pending')" :active="request()->routeIs('admin.schools.pending')">
                    {{ __('Pending Schools') }}
                </x-responsive-nav-link>
            ...
            {{-- School Responsive Navigation --}}
            @if(Auth::user()->role === 'SCHOOL')
                <x-responsive-nav-link :href="route('school.dashboard')" :active="request()->routeIs('school.dashboard')">
                    {{ __('Dashboard') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('school.players.index')" :active="request()->routeIs('school.players.*')">
                    {{ __('Players') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('school.profile.index')" :active="request()->routeIs('school.profile.*')">
                    {{ __('School Profile') }}
                </x-responsive-nav-link>
            @endif

            {{-- Player Responsive Navigation --}}
            @if(Auth::user()->role === 'PLAYER')
